  ...
  result <- foreach(i = 1:length(peers), .combine = rbind) %dopar% {
    ...
    for(j in 2:nrow(x)){
      ...
      if(x[j-1,]$unat>0){
        ...
        # print(paste("j: ",j,sep=""))
        # print(paste("unattended: ",unattended,sep=""))
        # if(j>5){
        #   break
        # }
      }
...
png(paste(path$cycle,"satisfacao-lambda10-semcarona.png",sep=""), width=800, height=350)
...
library(ggplot2)
path$cycle <- paste(paste(path$exp,cycle,sep=""),"/",sep="")
png(paste(path$cycle,"fairness-lambda10-semcarona.png",sep=""), width=800, height=315)
